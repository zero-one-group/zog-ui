...
class ZeroDropdownMenuEntry<T> {
  ...
  ///
  /// The default value is true. If true, the [ZeroDropdownMenuEntry.label] will be filled
...
///
/// The menu is composed of a list of [ZeroDropdownMenuEntry]s. People can provide information,
...
/// * [ZeroDropdownMenuEntry], which is used to build the [MenuItemButton] in the [CustomDropdownMenu] list.
class CustomDropdownMenu<T> extends StatefulWidget {
  ...
  ///
  /// This is a required parameter. It is recommended that at least one [ZeroDropdownMenuEntry]
...
class _CustomDropdownMenuState<T> extends State<CustomDropdownMenu<T>> {
  ...
  late List<ZeroDropdownMenuEntry<T>> filteredEntries;
  ...
  void initState() {
    ...
    _menuHasEnabledItem =
        filteredEntries.any((ZeroDropdownMenuEntry<T> entry) => entry.enabled);

    final int index = filteredEntries.indexWhere(
        (ZeroDropdownMenuEntry<T> entry) =>
  ...
  void didUpdateWidget(CustomDropdownMenu<T> oldWidget) {
    ...
    if (oldWidget.dropdownMenuEntries != widget.dropdownMenuEntries) {
      _menuHasEnabledItem = filteredEntries
          .any((ZeroDropdownMenuEntry<T> entry) => entry.enabled);
    }
  ...
  List<ZeroDropdownMenuEntry<T>> filter(List<ZeroDropdownMenuEntry<T>> entries,
      TextEditingController textEditingController) {
    final String filterText = textEditingController.text.toLowerCase();
    return entries
        .where((ZeroDropdownMenuEntry<T> entry) =>
            entry.label.toLowerCase().contains(filterText))
        .toList();
  }

  int? search(List<ZeroDropdownMenuEntry<T>> entries,
      TextEditingController textEditingController) {
    final String searchText = textEditingController.value.text.toLowerCase();
    if (searchText.isEmpty) {
      return null;
    }
    final int index = entries.indexWhere((ZeroDropdownMenuEntry<T> entry) =>
        entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(List<ZeroDropdownMenuEntry<T>> filteredEntries,
    ...
    for (int i = 0; i < filteredEntries.length; i++) {
      final ZeroDropdownMenuEntry<T> entry = filteredEntries[i];
      ButtonStyle effectiveStyle = entry.style?.toButtonStyle() ?? defaultStyle;
  ...
  Widget build(BuildContext context) {
    ...
    MenuStyle? effectiveMenuStyle = widget.menuStyle?.toMenuStyle() ??
        theme.menuStyle ??
        defaults.menuStyle!;
                ...
                TextField(
                    ...
                    onEditingComplete: () {
                      if (currentHighlight != null) {
                        final ZeroDropdownMenuEntry<T> entry =
