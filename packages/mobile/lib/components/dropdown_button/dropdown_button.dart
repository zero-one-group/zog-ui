...
///
/// Used by [ZeroDropdownButton.selectedItemBuilder].
typedef ZeroDropdownButtonBuilder = List<Widget> Function(BuildContext context);
...
class _ZeroDropdownMenuItemButton<T> extends StatefulWidget {
  const _ZeroDropdownMenuItemButton({
    super.key,
    this.padding,
    required this.route,
  ...
  @override
  _ZeroDropdownMenuItemButtonState<T> createState() =>
      _ZeroDropdownMenuItemButtonState<T>();
}

class _ZeroDropdownMenuItemButtonState<T>
    extends State<_ZeroDropdownMenuItemButton<T>> {
  ...
  void _handleOnTap() {
    final ZeroDropdownMenuItem<T> dropdownMenuItem =
...
class _DropdownRoutePage<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // ZeroDropdownButton.itemHeight is specified or ZeroDropdownButton.itemHeight is null
...
// The container widget for a menu item created by a [ZeroDropdownButton]. It
// provides the default configuration for [ZeroDropdownMenuItem]s, as well as a
// [ZeroDropdownButton]'s hint and disabledHint widgets.
class _ZeroDropdownMenuItemContainer extends StatelessWidget {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const _ZeroDropdownMenuItemContainer({
    super.key,
...
class ZeroDropdownMenuItem<T> extends _ZeroDropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
...
/// [ZeroDropdownButton] widgets placed within material data tables, as
/// required by the Material Design specification.
class ZeroDropdownButtonHideUnderline extends InheritedWidget {
  /// Creates a [ZeroDropdownButtonHideUnderline]. A non-null [child] must
  /// be given.
  ...
  @override
  bool updateShouldNotify(ZeroDropdownButtonHideUnderline oldWidget) => false;
}
...
/// Typically, an enum is used. Each [ZeroDropdownMenuItem] in [items] must be
...
///
/// {@tool dartpad}
/// This sample shows a [ZeroDropdownButton] with a large arrow icon,
...
/// See also:
///
///  * [ZeroDropdownButtonFormField], which integrates with the [Form] widget.
///  * [ZeroDropdownMenuItem], the class used to represent the [items].
///  * [ZeroDropdownButtonHideUnderline], which prevents its descendant dropdown buttons
///    from displaying their underlines.
///  * [ElevatedButton], [TextButton], ordinary buttons that trigger a single action.
///  * <https://material.io/design/components/menus.html#dropdown-menu>
class ZeroDropdownButton<T> extends StatefulWidget {
  /// Creates a dropdown button.
  ///
  /// The [items] must have distinct values. If [value] isn't null then it
  /// must be equal to one of the [ZeroDropdownMenuItem] values. If [items] or
  ...
  ZeroDropdownButton({
    ...
    // ZeroDropdownButtonFormField.
  })  : assert(
          items == null ||
              items.isEmpty ||
              value == null ||
              items.where((ZeroDropdownMenuItem<T> item) {
                    return item.value == value;
                  }).length ==
                  1,
          "There should be exactly one item with [ZeroDropdownButton]'s value: "
          '$value. \n'
    ...
    required bool isFocused,
          ...
          items == null ||
              ...
              items.where((ZeroDropdownMenuItem<T> item) {
                  ...
                  1,
          "There should be exactly one item with [ZeroDropdownButtonFormField]'s value: "
          '$value. \n'
          'Either zero or 2 or more [ZeroDropdownMenuItem]s were detected '
          'with the same value',
        ),
  ...
  final List<ZeroDropdownMenuItem<T>>? items;

  /// The value of the currently selected [ZeroDropdownMenuItem].
  ///
  /// If [value] is null and the button is enabled, [hint] will be displayed
  /// if it is non-null.
  ///
  ...
  ///
  /// If the [onChanged] callback is null or the list of [ZeroDropdownButton.items]
  /// is null then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [ZeroDropdownButton.disabledHint] widget if it is non-null.
  /// If [ZeroDropdownButton.disabledHint] is also null but [ZeroDropdownButton.hint] is
  /// non-null, [ZeroDropdownButton.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  /// [ZeroDropdownMenuItem]s in [items].
  ///
  /// When a [ZeroDropdownMenuItem] is selected, the widget that will be displayed
  /// from the list corresponds to the [ZeroDropdownMenuItem] of the same index
  /// in [items].
  ///
  /// {@tool dartpad}
  /// This sample shows a `ZeroDropdownButton` with a button with [Text] that
  /// corresponds to but is unique from [ZeroDropdownMenuItem].
  ///
  /// ** See code in examples/api/lib/material/dropdown/dropdown_button.selected_item_builder.0.dart **
  /// {@end-tool}
  ///
  /// If this callback is null, the [ZeroDropdownMenuItem] from [items]
  ...
  ///
  /// {@tool dartpad}
  /// This sample shows a `ZeroDropdownButton` with a dropdown button text style
  ...
  @override
  State<ZeroDropdownButton<T>> createState() => _ZeroDropdownButtonState<T>();
}

class _ZeroDropdownButtonState<T> extends State<ZeroDropdownButton<T>>
  ...
  @override
  void didUpdateWidget(ZeroDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    if (widget.items == null ||
        ...
        (widget.value == null &&
            widget.items!
                .where((ZeroDropdownMenuItem<T> item) =>
    ...
    assert(widget.items!
            .where((ZeroDropdownMenuItem<T> item) => item.value == widget.value)
            .length ==
        1);
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.hint != null || (!_enabled && widget.disabledHint != null)) {
      ...
      items.add(DefaultTextStyle(
        ...
        child: IgnorePointer(
          ...
          child: _ZeroDropdownMenuItemContainer(
    ...
    if (!ZeroDropdownButtonHideUnderline.at(context)) {
      final double bottom =
...
///
/// This is a convenience widget that wraps a [ZeroDropdownButton] widget in a
/// [FormField].
///
...
/// See also:
///
///  * [ZeroDropdownButton], which is the underlying text field without the [Form]
///    integration.
class ZeroDropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [ZeroDropdownButton] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  ZeroDropdownButtonFormField({
    super.key,
    required List<ZeroDropdownMenuItem<T>>? items,
    ...
    // ZeroDropdownButton.
        ...
        super(
          ...
          builder: (FormFieldState<T> field) {
            final _ZeroDropdownButtonFormFieldState<T> state =
                field as _ZeroDropdownButtonFormFieldState<T>;
            ...
            return Focus(
              ...
              child: Builder(builder: (BuildContext context) {
                return ZeroDropdownButtonHideUnderline(
                  child: ZeroDropdownButton<T>._formField(
                    items: items,
  ...
  @override
  FormFieldState<T> createState() => _ZeroDropdownButtonFormFieldState<T>();
}

class _ZeroDropdownButtonFormFieldState<T> extends FormFieldState<T> {
  @override
  void didChange(T? value) {
    super.didChange(value);
    final ZeroDropdownButtonFormField<T> dropdownButtonFormField =
        widget as ZeroDropdownButtonFormField<T>;
    assert(dropdownButtonFormField.onChanged != null);
    dropdownButtonFormField.onChanged!(value);
  }

  @override
  void didUpdateWidget(ZeroDropdownButtonFormField<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
