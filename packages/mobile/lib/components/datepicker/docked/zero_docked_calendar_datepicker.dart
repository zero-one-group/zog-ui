...
class ZeroDockedCalendarDatePicker extends StatefulWidget {
  ...
  final ZeroDatePickerMode initialCalendarMode;
  ...
  State<ZeroDockedCalendarDatePicker> createState() =>
      _ZeroDockedCalendarDatePickerState();
}

class _ZeroDockedCalendarDatePickerState
    extends State<ZeroDockedCalendarDatePicker> {
  bool _announcedInitialDate = false;
  late ZeroDatePickerMode _mode;
  ...
  @override
  void didUpdateWidget(ZeroDockedCalendarDatePicker oldWidget) {
  ...
  void _handleModeChanged(ZeroDatePickerMode mode) {
    ...
    setState(() {
      _mode = mode;
      if (_mode == ZeroDatePickerMode.day) {
        SemanticsService.announce(
  ...
  Widget _buildPicker() {
    switch (_mode) {
      ...
      case ZeroDatePickerMode.year:
        return Padding(
          padding: const EdgeInsets.only(top: _subHeaderHeight),
          child: DockedYearPicker(
            key: _yearPickerKey,
            currentDate: widget.currentDate,
            firstDate: widget.firstDate,
            lastDate: widget.lastDate,
            ...
            Flexible(
              ...
              child: Container(
                margin: const EdgeInsets.symmetric(
                    horizontal: _kYearNavButtonIconSize),
                child: _DatePickerModeToggleButton(
                  ...
                  onTitlePressed: () {
                    ...
                    _handleModeChanged(_mode != ZeroDatePickerMode.month
                        ? ZeroDatePickerMode.month
                        : ZeroDatePickerMode.day);
                  },
                ),
              ),
            ),
...
/// A button that used to toggle the [ZeroDatePickerMode] for a date picker.
///
...
class _DatePickerModeToggleButtonState
  ...
  void initState() {
    ...
    _controller = AnimationController(
      value: widget.mode == ZeroDatePickerMode.year ? 0.5 : 0,
      upperBound: 0.5,
  ...
  void didUpdateWidget(_DatePickerModeToggleButton oldWidget) {
    ...
    if (widget.mode == ZeroDatePickerMode.year) {
      _controller.forward();
    } else {
      _controller.reverse();
    }
  ...
  Widget build(BuildContext context) {
    final bool inactive =
        !widget.active && widget.mode != ZeroDatePickerMode.day;
          ...
          child: SizedBox(
            ...
            child: InkWell(
              ...
              child: Row(
                ...
                mainAxisSize:
                    fullWidthButton ? MainAxisSize.max : MainAxisSize.min,
                children: <Widget>[
                  Text(
                    widget.title,
...
class DockedMonthPicker extends StatefulWidget {
...
class _DockedMonthPickerState extends State<DockedMonthPicker> {
  late ScrollController _scrollController;

  // The approximate number of years necessary to fill the available space.
  static const int minYears = 18;

  static const int kMonthsLength = 12;

  @override
  void initState() {
    super.initState();
    _scrollController = ScrollController(
        initialScrollOffset: _scrollOffsetForMonth(widget.selectedDate));
  }

  @override
  void didUpdateWidget(DockedMonthPicker oldWidget) {
    super.didUpdateWidget(oldWidget);
  ...
  double _scrollOffsetForMonth(DateTime date) {
    ...
    final int centeredYearRow = initialYearIndex - 2;
...
class _DockedYearPickerState extends State<DockedYearPicker> {
  late ScrollController _scrollController;
  ...
  @override
  void didUpdateWidget(DockedYearPicker oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.selectedDate != oldWidget.selectedDate) {
  ...
  Widget _buildYearItem(BuildContext context, int index) {
    // Backfill the _DockedYearPicker with disabled years if necessary.
