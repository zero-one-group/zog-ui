...
export const CustomSort = ({ sort }) => {
  return (
    <div style={{ color: 'var(--colors-primary9)' }}>
      {sort ? (
        sort === 'asc' ? (
          <SortAscendingOutlined />
        ) : (
...
<Canvas columns={2} withSource="close">
  ...
  <Story
    ...
    args={{
      ...
      enableSorting: true,
      enableSelection: true,
      sortIcon: (sort) => <CustomSort sort={sort} />,
    ...
    args={{
      ...
      css: {
        '& th': {
          background: '$pink9',
          color: 'white',
          '& [data-sort="asc"] span:first-child svg, [data-sort="desc"] span:last-child svg':
            {
              color: '$pink1',
            },
        },
